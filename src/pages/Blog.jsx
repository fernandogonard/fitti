...
const posts = [
  ...
  },
  {
    ...
    image: testimonial1,
    date: "25/10/2025",
    excerpt: "'Con Fittipaldi Gloves siento seguridad y agarre en cada partido. La diferencia se nota desde el primer uso.' - Lucas G., portero Primera B Nacional.",
    content: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse euismod, urna eu tincidunt consectetur, nisi nisl aliquam enim, euismod cursus massa enim nec dui. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas."
  },
  {
  ...
  {
    title: "Historia de la marca Fittipaldi",
    image: gloveElite,
    date: "20/10/2025",
    excerpt: "Conoce cómo nació Fittipaldi Gloves y la pasión detrás de cada diseño.",
    content: "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse euismod, urna eu tincidunt consectetur, nisi nisl aliquam enim, euismod cursus massa enim nec dui. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas."
  },
  {
    ...
    <BackgroundWrapper>
      <main className="min-h-screen w-full px-4 py-12 flex flex-col items-center justify-center">
        <section className="max-w-4xl w-full mb-12 text-center">
          <h1 className="font-display text-4xl md:text-5xl font-extrabold text-orange mb-4 drop-shadow-lg tracking-tight">Noticias y Novedades</h1>
          <p className="text-lg md:text-xl text-gray-200 font-medium mb-6">Mantente al día con los lanzamientos, consejos y testimonios de porteros que confían en Fittipaldi Gloves.</p>
        </section>
        <section className="grid grid-cols-1 md:grid-cols-2 gap-10 w-full max-w-5xl">
          {posts.map((post, idx) => (
            <article key={idx} className="bg-white rounded-2xl shadow-xl p-6 flex flex-col items-center border border-gray-100 mb-8">
              <img src={post.image} alt={post.title} className="w-32 h-32 object-cover rounded-xl mb-4 shadow-md" />
              <h2 className="font-display text-xl font-bold text-black mb-2 text-center">{post.title}</h2>
              <span className="text-orange font-bold text-sm mb-2">{post.date}</span>
              <p className="text-black text-base mb-2 text-center font-medium">{post.excerpt}</p>
              <details className="w-full mt-2">
                <summary className="cursor-pointer text-orange font-semibold mb-2">Leer más</summary>
                <div className="text-gray-700 text-sm mt-2 px-2 pb-2 text-left">
                  {post.content}
                </div>
              </details>
              <a href="/catalog" className="inline-block bg-orange text-black font-bold py-2 px-6 rounded-lg shadow-md hover:bg-gold transition-colors text-sm mt-4">Ver producto</a>
            </article>
          ))}
        </section>
      </main>
